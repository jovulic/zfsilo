...
import (
	...
	converter "github.com/jovulic/zfsilo/app/internal/converter"
...
func (c *VolumeConverterImpl) FromAPIToDB(source *v1.Volume) (database.Volume, error) {
	...
	if source != nil {
		...
		datatypesJSON, err := converter.ConvertFromStructToJSON((*source).Struct)
		if err != nil {
			return databaseVolume, err
		}
		databaseVolume2.Struct = datatypesJSON
		timeTime, err := converter.ConvertTimestampToTime((*source).CreateTime)
		if err != nil {
			return databaseVolume, err
		}
		databaseVolume2.CreateTime = timeTime
		timeTime2, err := converter.ConvertTimestampToTime((*source).UpdateTime)
		if err != nil {
			return databaseVolume, err
		}
		...
		databaseVolume2.Options = converter.ConvertVolumeOptionsFromAPIToDB((*source).Options)
		databaseVolume2.Sparse = (*source).Sparse
		databaseVolume2.Mode = converter.ConvertVolumeModeFromAPIToDB((*source).Mode)
...
func (c *VolumeConverterImpl) FromDBToAPI(source database.Volume) (*v1.Volume, error) {
	...
	pStructpbStruct, err := converter.ConvertFromJSONToStruct(source.Struct)
	if err != nil {
		return nil, err
	}
	zfsilov1Volume.Struct = pStructpbStruct
	pTimestamppbTimestamp, err := converter.ConvertTimeToTimestamp(source.CreateTime)
	if err != nil {
		return nil, err
	}
	zfsilov1Volume.CreateTime = pTimestamppbTimestamp
	pTimestamppbTimestamp2, err := converter.ConvertTimeToTimestamp(source.UpdateTime)
	if err != nil {
		return nil, err
	}
	...
	zfsilov1Volume.Options = converter.ConvertVolumeOptionsFromDBToAPI(source.Options)
	zfsilov1Volume.Sparse = source.Sparse
	zfsilov1Volume.Mode = converter.ConvertVolumeModeFromDBToAPI(source.Mode)
